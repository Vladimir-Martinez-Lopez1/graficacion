export class CanvasLocal {
    ...
    drawRmboide(x1, y1, x2, y2, x3, y3, x4, y4, color, drawBorder = false) {
        // Color de relleno
    ...
    maxH(h) {
        if (h.length === 0)
            return 10; // Valor por defecto cuando no hay datos
    ...
    barra(x, y, alt, anchoLados = 0.5) {
        ...
        const color = this.graphics.strokeStyle;
    ...
    paint() {
        ...
        this.barrasInfo = [];
        //color ya generado
        const colors = Array.from({ length: this.h.length }, () => generarColores());
...
export function setupChartControls(canvasLocal) {
    //constantes para los elementos del formulario
    const datosRecibidos = document.getElementById("datosRecibidos");
